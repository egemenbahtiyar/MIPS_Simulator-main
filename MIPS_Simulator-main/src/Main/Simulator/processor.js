...
processor.updateCache = (wordAddress, store) => {
    ...
    for (
        let i = 0;
        i < l1_blocks;
        i++ //parsing through the blocks in the corresponding set
    ) {
        if (processor.L1Tags.get([l1set_index, i]) == l1block_index) {
            ...
            if (processor.L1Priority.get([l1set_index, i]) != 0) {
                for (
                    let j = 0;
                    j < l1_blocks;
                    j++ //parsing through the blocks in the corresponding set and updating priority
                ) {
                    if (
                        processor.L1Priority.get([l1set_index, j]) != -1 &&
                        processor.L1Priority.get([l1set_index, j]) < currentP1
                    ) {
            ...
            if (store) {
                for (
                    let j = 0;
                    j < l1_block_size;
                    j++ //parsing through the block and updating L1 data because it is a store
                ) {
                    let t = l1block_index * l1_block_size;
                    processor.L1.set(
                        [l1set_index, i, j],
                        processor.memory[t + j]
                    );
                }
            } else {
                return;
    ...
    if (!l1_flag) {
        //if the search was unsuccessful, need to write/overwrite
        for (
            let i = 0;
            i < l1_blocks;
            i++ //parsing through the blocks in the corresponding set to update remaining priorities
        ) {
        ...
        for (
            ...
            if (
                processor.L1Priority.get([l1set_index, i]) == -1 ||
                processor.L1Priority.get([l1set_index, i]) > l1_blocks - 1
            ) {
                processor.L1Priority.set([l1set_index, i], 0);
    ...
    for (
        ...
        if (processor.L2Tags.get([l2set_index, i]) == l2block_index) {
            ...
            if (
                processor.L2Priority.get([l2set_index, i]) != 0 &&
                (store || !l1_flag)
            ) {
                //only update priority if it is not a L1 hit or if it is a store
                for (
                    let j = 0;
                    j < l2_blocks;
                    j++ //parsing through the blocks in the corresponding set and updating priority
                    ...
                    if (
                        processor.L2Priority.get([l2set_index, j]) != -1 &&
                        processor.L2Priority.get([l2set_index, j]) < currentP2
                    ) {
            ...
            if (store) {
                for (
                    let j = 0;
                    j < l2_block_size;
                    j++ //parsing through the block and updating data because it is a sw
                ) {
    ...
    if (!l2_flag) {
        ...
        for (
            ...
            if (
                processor.L2Priority.get([l2set_index, i]) == -1 ||
                processor.L2Priority.get([l2set_index, i]) > l2_blocks - 1
            ) {
                processor.L2Priority.set([l2set_index, i], 0);
